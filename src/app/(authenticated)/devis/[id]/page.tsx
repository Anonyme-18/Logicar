...
export default function QuoteEditorPage() {
  ...
  const { data: editor, isPending } = useQuery({
    ...
    queryFn: () => getQuoteEditor({ data: quoteId ? { id: quoteId } : {} }),
  });
  const { data: quoteList } = useQuery({ queryKey: ["quotes"], queryFn: () => listQuotes({}) });
  ...
  const { data: preview, isFetching: previewing } = useQuery({
    ...
    queryFn: () => renderQuotePreview({ data: debounced }),
    enabled: !isPending,
  ...
  async function handleStatus(next: "SENT" | "ACCEPTED" | "REJECTED") {
    if (!quoteId) return;
    try {
      await setQuoteStatus({ data: { id: quoteId, status: next } });
      await queryClient.invalidateQueries({ queryKey: ["quote-editor", quoteId] });
      await queryClient.invalidateQueries({ queryKey: ["quotes"] });
  ...
  async function handleConvert() {
    ...
    setConverting(true);
    try {
      const { id: invoiceId } = await convertQuote({ data: { quote_id: quoteId } });
      await queryClient.invalidateQueries({ queryKey: ["quotes"] });
                ...
                <li key={q.id}>
                  ...
                  >
                    ...
                    <span className="text-xs text-muted-foreground">
                      {QUOTE_STATUS[q.status as QuoteStatus]?.label ?? q.status} ·{" "}
