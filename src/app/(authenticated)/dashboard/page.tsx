...
import { ArrowUpRight, Clock, FileCheck2, Plus, Search, TrendingUp, Wallet } from "lucide-react";
...
export default function DashboardPage() {
  ...
  const { data: quotes } = useQuery({
    ...
    queryFn: () => listQuotes({ data: undefined }),
  });
    ...
    <AppShell>
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-12">
        ...
        <section className="rounded-3xl bg-card p-5 shadow-glass sm:p-6 lg:col-span-4">
          ...
          <p className="mt-0.5 text-xs text-muted-foreground">Facturé et encaissé sur la période</p>
        ...
        <section className="rounded-3xl bg-card p-5 shadow-glass sm:p-6 lg:col-span-12">
          <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-4 sm:flex sm:justify-between">
            <h2 className="min-w-0 truncate font-display text-base font-semibold">Devis récents</h2>
                    ...
                    return (
                      <tr key={q.id} className="transition-colors hover:bg-muted/50">
                        <td className="rounded-l-xl px-4 py-3 font-medium tabular">
                          <Link href={`/devis/${q.id}`}>{q.quote_number}</Link>
                        </td>
